...
import { 
    Fn, int, atomicAdd, storage, instanceIndex, If, float, 
    atomicMin, uniform, uint, max, min, abs 
} from 'three/tsl';
import { SurfelStruct, type SurfelPool } from './surfelPool';
import { 
    SURFEL_LIFE_RECYCLE, SURFEL_LIFE_RECYCLED, 
    TOTAL_CELLS, MAX_SURFELS_PER_CELL_FOR_KEEP_ALIVE, 
    SURFEL_MAX_HEALTH, SURFEL_KILL_SIGNAL, 
    SURFEL_TTL,
...
export function createSurfelAgePass(): SurfelAgePass {
  ...
  function run(
    ...
    if (!surfelAttr || !poolAlloc || !poolMax || !touchedAtomic || !offsetsAndListAttr|| !poolAttr || !execAttr) return;
    ...
    if (!computeNode) {
        ...
        const offsetsAndListStore = storage(offsetsAndListAttr, 'int', offsetsAndListAttr.count).toReadOnly();
        ...
        })().computeKernel([64, 1, 1]).setName('Surfel Economy');
    }
