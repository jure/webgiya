...
export function createSceneBVH(renderer: THREE.WebGPURenderer, scene: THREE.Scene): SceneBVHBundle {

    const geometries: THREE.BufferGeometry[] = [];
    scene.updateMatrixWorld(true);
    
    const { diffuseArrayTex, materialIdByUUID } = buildDiffuseArrayTexture(scene, 1024);
    ...
    scene.traverse((obj) => {
      if (obj instanceof THREE.Mesh && obj.visible) {
        ...
        if (!geom.getAttribute('uv')) {
          geom.setAttribute('uv', new THREE.BufferAttribute(new Float32Array(vertexCount * 2), 2));
        }
    ...
    const bvhAttr = new THREE.StorageBufferAttribute(new Float32Array(bvh._roots[0]), 8); // BVHNode is 8 floats
    const posAttr = new THREE.StorageBufferAttribute(merged.attributes.position.array, 3);
    const norAttr = new THREE.StorageBufferAttribute(merged.attributes.normal.array, 3);
    ...
    const positionNode = storage(posAttr, 'vec3', 0).toReadOnly().setName('bvh_position');
    const normalNode = storage(norAttr, 'vec3', 0).toReadOnly();
    const indexNode = storage(idxAttr, 'uvec3', 0).toReadOnly().setName('bvh_index');
    const colorNode = storage(colAttr, 'vec3', 0).toReadOnly().setName('bvh_attribute');
    ...
    const resizeAndUpload = (attr: THREE.StorageBufferAttribute, data: ArrayLike<number>, itemSize: number) => {
        if (attr.array.length < data.length) {
            attr.array = data instanceof Float32Array ? new Float32Array(data) : new Uint32Array(data);
            // @ts-ignore
  ...
  // Initial empty
  return { bvhNode, positionNode, normalNode, indexNode, colorNode, diffuseArrayTex };
}
