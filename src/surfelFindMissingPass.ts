...
import { MAX_SURFELS_PER_CELL, MAX_SURFELS_PER_CELL_FOR_KEEP_ALIVE, OFFSETS_AND_LIST_START, SURFEL_KILL_SIGNAL, SURFEL_LIFE_RECYCLE, SURFEL_RADIUS_OVERSCALE, TOTAL_CELLS } from './constants';
...
export function createSurfelFindMissingPass(): SurfelFindMissingPass {
  ...
  const candPackedAttr = new THREE.StorageBufferAttribute(new Float32Array(4), 4);
  const tileIrradianceAttr = new THREE.StorageBufferAttribute(new Float32Array(4), 4);
  ...
  function run(
    ...
    if (!poolAttr || !offsetsAndListAttr || !momentsAttr || !touchedAtomic) return { tileCount: 0 };
    ...
    if (poolAttr !== lastPoolAttr || offsetsAndListAttr !== lastGridAttr || tileCount !== lastTileCount) {
      computeNode = null;
    ...
    if (!computeNode) {
      ...
      const candPacked = storage(candPackedAttr, VertexPacked, candPackedAttr.count);
      ...
      const computeLogic = Fn(() => {
        ...
        If(pixelInBounds, () => {
          ...
          const uv = vec2(pxF.add(float(0.5)).div(widthF), pyF.add(float(0.5)).div(heightF));

          // --- Random ---
          const seed = hashCombine2(hashCombine2(uint(px), hash1(uint(py))), uint(U_FRAME)).toVar();
          ...
          If(valid, () => {
            ...
            Loop(loopCount, ({ i }) => {
              const sid = offsetsAndList.element(int(OFFSETS_AND_LIST_START).add(baseIdx).add(i));
              ...
              const surfel_c4_hash = surfel_grid_coord_to_hash(surfel_pos_to_grid_coord(surfel_pRel))
              ...
              const alignPenalty = abs(dot(posOffset, surfelNormal)).mul(SURFEL_NORMAL_DIRECTION_SQUISH);
              const mahal = d.mul(float(1.0).add(alignPenalty));
              const dotN = surfelNormal.dot(normal).max(float(0.0));
  
              const weight = smoothstep(surfelRadius.mul(SURFEL_RADIUS_OVERSCALE), float(0.0), mahal).mul(dotN);
              ...
              // At least 8 frames
              const isMature = float(U_FRAME).sub(posb.w).greaterThanEqual(float(8.0)).and(dotN.greaterThan(0.9))
              ...
              // Keep-alive
              const sameCellKeep = cnt.lessThanEqual(maxKeepAlive).and(dotN.greaterThan(float(0.8))).and(isSameHash);
            ...
            const shouldConsiderDespawn = scoringWeight.greaterThan(despawnThreshold).and(second.greaterThan(secondThresh));
            ...
            const prob = probMult.mul(depthWeight).mul(widthF.reciprocal()).mul(heightF.reciprocal());
            const randomOk = uintToU01Float(hash1_mut(seed)).lessThan(prob);
            const gateCoverage = second.lessThan(float(0.4)).and(scoringWeight.lessThan(float(0.1))).and(cnt.lessThan(maxKeepAlive));
            ...
            If(wantSpawn, () => {
               ...
               const packedVal = bitOr(bitAnd(scoreAsUint, uint(0xFFFFFFC0)), uint(laneIndex));
        ...
        If(maxScorePacked.notEqual(uint(0)), () => {
            ...
            If(laneIndex.equal(winningLane), () => {
                ...
                Loop(loopCount, ({ i }) => {
                    ...
                    const sid = offsetsAndList.element(int(OFFSETS_AND_LIST_START).add(startIdx).add(i));
                    const packed = surfels.element(sid);
                    const posb = packed.get('posb');
                    const surfelNormal = packed.get('normal');
                    ...
                    const w = smoothstep(surfelRadius.mul(SURFEL_RADIUS_OVERSCALE), float(0.0), mahal).mul(dirW);
                ...
                If(victimIdx.notEqual(int(-1)), () => {
                  atomicMax(touchedAtomic.element(victimIdx), int(SURFEL_KILL_SIGNAL))
                  const base = tileIndex.mul(int(2));
        ...
        If(bestPacked.notEqual(uint(0xFFFFFFFF)), () => {
          ...
          If(laneIndex.equal(winningLaneIndex), () => {
            ...
            If(accumIrrWeight.greaterThan(float(1e-5)), () => {
              ...
              tileIrradiance.element(tileIndex).assign(vec4(avgIrr, initSamples));
            });
           });
        });
      ...
      computeNode = computeLogic().computeKernel([GROUP_SIZE_X, GROUP_SIZE_Y, 1]).setName('Surfel Find Missing');
