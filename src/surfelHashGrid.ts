...
import {
  Fn, storage, float, int, uint, instanceIndex,
  atomicAdd,
  workgroupArray, workgroupBarrier, localId, workgroupId,
  Loop, bitAnd, If, min, max,
  instancedArray, abs, floor, clamp, log2, ceil,
  vec3, ivec3, uvec4, uniform,
  length,
  select,
  struct,
  dot,
...
import { CASCADES, MAX_SURFELS_PER_CELL, OFFSETS_AND_LIST_START, SURFEL_BASE_RADIUS, SURFEL_CS, SURFEL_GRID_CELL_DIAMETER, SURFEL_NORMAL_DIRECTION_SQUISH, SURFEL_TTL, TOTAL_CELLS } from './constants';

export type SurfelHashGrid = {
  build: (renderer: THREE.WebGPURenderer, pool: SurfelPool, camera: THREE.PerspectiveCamera) => void;
...
export const get_surfel_grid_box_min_max = Fn(([pRel]: [THREE.Node]) => {
  ...
  const result = SurfelGridMinMax({
    c0_min: uvec4(uint(clampToGrid(minC0).x), uint(clampToGrid(minC0).y), uint(clampToGrid(minC0).z), c0),
    c0_max: uvec4(uint(clampToGrid(maxC0).x), uint(clampToGrid(maxC0).y), uint(clampToGrid(maxC0).z), c0),
    c1_min: uvec4(uint(clampToGrid(minC1).x), uint(clampToGrid(minC1).y), uint(clampToGrid(minC1).z), c1),
    c1_max: uvec4(uint(clampToGrid(maxC1).x), uint(clampToGrid(maxC1).y), uint(clampToGrid(maxC1).z), c1),
...
const surfel_grid_coord_within_cascade = Fn(([coord, cascade]: [THREE.Node, THREE.Node]) => {
  return coord.shiftRight(ivec3(cascade)).add(int(SURFEL_CS).div(2)); 
});

// Main: Calculate Center of a Grid Cell (World Space)
export const surfel_grid_coord_center = Fn(([coord, eye_pos]: [THREE.Node, THREE.Node]) => {
...
export const surfel_radius_for_pos = Fn(([pos, camPos]: [THREE.Node, THREE.Node]) => {
  const dist = length(pos.sub(camPos));
  const cascadeRadius = float(SURFEL_GRID_CELL_DIAMETER).mul(SURFEL_CS).mul(0.5); // like SURFEL_GRID_CASCADE_RADIUS
  // return 0.1;
...
export const surfel_intersects_grid_coord = Fn(([surfelWorldPos, normal, radius, c4, gridOrigin]: [THREE.Node, THREE.Node, THREE.Node, THREE.Node, THREE.Node]) => {
  ...
  const cellRadius = float(SURFEL_GRID_CELL_DIAMETER).mul(0.5).mul(cascadeScale);
  ...
  const mahalanobisDist = distLen.mul(float(1.0).add(dotNormal.mul(SURFEL_NORMAL_DIRECTION_SQUISH)));
...
export function createSurfelHashGrid(): SurfelHashGrid {
  ...
  function ensure() {
    ...
    if (!segmentSumAttr || segmentSumAttr.count !== segments) {
      segmentSumAttr = new THREE.StorageBufferAttribute(new Int32Array(segments), 1);
      buffersRecreated = true;
    }
  ...
  function build(
    ...
    if (!computeClear) {
        ...
        const offsetsAndList = storage(offsetsAndListAtomic!.value, 'int', offsetsAndListAtomic?.value.count);
        // const list = storage(cellToSurfelAttr!, 'int', cellToSurfelAttr!.count);
        const segSums = segmentSumAttr ? storage(segmentSumAttr, 'int', segments) : null;
        ...
        })().compute(Math.max(1, totalCells + 1)).setName('Grid Clear');
                      ...
                      Loop({ start: int(currentMin.y), end: int(currentMax.y).add(1), name: 'y', type: 'int' }, ({ y }) => {
                          Loop({ start: int(currentMin.x), end: int(currentMax.x).add(1), name: 'x', type: 'int' }, ({ x }) => {
                              ...
                              If(intersects, () => {
                                  ...
                                  atomicAdd(offsetsAndListAtomic!.element(hash), int(1));
                              });
                          });
                      });
                  });
        ...
        })().compute(capacity).setName('Grid Count');
        ...
        computeScanSeg = Fn(() => {
            ...
            shared.element(idx2).assign(in0.select(offsetsAndList.element(g0), int(0)));
            shared.element(idx2.add(int(1))).assign(in1.select(offsetsAndList.element(g1), int(0)));
            workgroupBarrier();
            ...
            If(in0, () => { offsetsAndList.element(g0).assign(shared.element(idx2))});
            If(in1, () => { offsetsAndList.element(g1).assign(shared.element(idx2.add(int(1))))});
        ...
        })().compute(1).setName('Grid Seg Prefix');
        ...
        computeMerge = Fn(() => {
            ...
            If(inRange.and(hasPrev), () => {
                offsetsAndList.element(idx).assign(offsetsAndList.element(idx).add(prevSum));
            });
                      ...
                      Loop({ start: int(currentMin.y), end: int(currentMax.y).add(1), name: 'y', type: 'int' }, ({ y }) => {
                          Loop({ start: int(currentMin.x), end: int(currentMax.x).add(1), name: 'x', type: 'int' }, ({ x }) => {
                              ...
                              If(intersects, () => {
                                  ...
                                  const endPlusOne = atomicAdd(offsetsAndListAtomic!.element(hash), int(-1));
                                  const writeIdx = endPlusOne.sub(int(1));
                                  
                                  // Safety Check
                                  const absoluteIdx = int(OFFSETS_AND_LIST_START).add(writeIdx);
                                  const idxOK = writeIdx.greaterThanEqual(int(0)).and(absoluteIdx.lessThan(int(offsetsAndListAttr!.count)));
                                  
                                  If(idxOK, () => {
                                      atomicStore(offsetsAndListAtomic!.element(absoluteIdx), i);
                                  });
                              });
                          });
                      });
  ...
  function getOffsetsAndListAttr() { return offsetsAndListAttr }
  function getSegmentSumAttr() { return segmentSumAttr; }
...
export function snap_to_surfel_grid_origin(out: THREE.Vector3, pos: THREE.Vector3) {
